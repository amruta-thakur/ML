define([], function() {
    ...
    ScenerioTemplate.prototype.init = function(xmlName) {
		...
		$('#min').html('00');
            $('#sec').html('00');
			//alert($(ref.container).find('img.mobile-bg-img').length)
			/*$(ref.container).find('img.mobile-bg-img, img.desktop-bg-img').load(function(){
				$('.scenerio-bg').show();	
			})*/
        ...
        ScenerioTemplate.prototype.loadXML = function(xmlNameRef) {
            ...
            $.ajax({
                ...
                success: function(xml) {
					...
					var obj2 = $(ref.container).find('#learningContent');
					
					setTimeout(function() {
					try{
						window.shell.attachScrollBar(obj2, 8, 100, '#cccccc', '#0090DA');
						//$(ref.container).find('.scenerio-bg').find('.hint_popup').find('.hint-scroll').getNiceScroll().show();
						}catch(e){
						
						}
						$(ref.container).find('#learningContent').getNiceScroll().show();
					$(ref.container).find('#learningContent').getNiceScroll().resize();
        ...
        ScenerioTemplate.prototype.loadstartscreen = function() {
            ...
            $(ref.container).find('#startCall_btn').on('click', function() {
                ...
                $(ref.container).find('#learningContent').getNiceScroll().hide();
        ...
        ScenerioTemplate.prototype.loadpersonaudio = function() {
			...
			ref.audioElementPerson.ontimeupdate = function() {
			//console.log(ref.audioElementPerson.currentTime + " ::: " + ref.audioElementPerson.duration);
			
			
			if(ref.audioElementPerson.currentTime >= ref.audioElementPerson.duration){
				 /*console.log("aaaaaa " + Clock.totalSeconds);
				 var cutomTime = Math.floor(ref.audioElementPerson.duration);
                    var customseconds = parseInt(cutomTime % 60);
                    var customMinutes = Math.floor(cutomTime / 60 % 60);
        ...
        ScenerioTemplate.prototype.playAudio = function() {
			//alert("11111");
			if(ref.alreadyWorking == true){
			return;
			}
			
			setTimeout(function(){ ref.alreadyWorking = false }, 1000);
			$(this).hide();
            ...
            if (playBtnid == "endplayBtn") {
				...
				console.log(ref.personaudioCompletedArray[ref.situationID])
				if (ref.personaudioCompletedArray[ref.situationID] == true) {} else {
                Clock.start(audioDuration, ref.situationID);
				}
			
			}
            ref.alreadyWorking = true;
        ...
        ScenerioTemplate.prototype.cyuoptionscreen = function() {
            ...
            hint += '<h4 id="hint_heading">Consejo</h4>'
            ...
            positivefeedback += '<h2 id="positivefeedback_heading">Correcto</h2>';
            ...
            positivefeedback += '<div class="btn btn-info custom-btn" id="continue_withpositiveresponse_btn">Continuar</div>'
            negativefeedback += '<h2 id="negativefeedback_heading">Incorrecto</h2>';
            ...
            negativefeedback += '<div class="btn btn-info custom-btn" id="continue_withnegativeresponse_btn">Continuar</div>'
			...
			//img += '<div class="play-pause-btn"><div id="playBtnCyu"  style="display:none;" class="playBtn"><i class="fa fa-play"></i></div><div id="stopBtnCyu" class="stopBtn"><i class="fa fa-pause"></i></div></div>'
            ...
            $(ref.container).find(".cyu_image").find('.c_img').html(img);
        ...
        ScenerioTemplate.prototype.loadNextsituation = function() {
            ref.situationID++;
			//alert(ref.situationID + " ::: " + ref.situationNo) 
        ...
        ScenerioTemplate.prototype.endingConversationScreen = function() {
            ...
            ref.audioEndingElement.onloadedmetadata = function() {
                ...
                if (ref.endconversioncompleted == true) {
					...
					ref.audioEndingElement.ontimeupdate = function() {
			//console.log(ref.audioElementPerson.currentTime + " ::: " + ref.audioElementPerson.duration);
			
			
			if(ref.audioEndingElement.currentTime >= ref.audioEndingElement.duration){
        ...
        ScenerioTemplate.prototype.showdetailscreen = function() {
            ...
            for (var k = 0; k <= ref.showdetailsNo - 1; k++) {
                str += '<div class="main_elem">'
                if (this.correctsituationAnswer[k] == 1) {
                    str += '<div class="elem">' + $(ref.xml).find('showdetailssituations').find('showdetails').eq(k).text() + '</div>'
                    str += '<div class="elem"><img src="templates/scenerio/images/thumbsUp.png"/></div>'
                } else {
                    str += '<div class="elem" style="color:#DB0A5B;">' + $(ref.xml).find('showdetailssituations').find('showdetails').eq(k).text() + '</div>'
                    str += '<div class="elem"><img src="templates/scenerio/images/thumbsDown.png"/></div>'
                }
                str += "</div>"
            }
