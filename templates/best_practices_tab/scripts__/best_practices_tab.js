    ...
    bestPracticesTemplate.prototype.init = function(xmlName) {
        ...
        $.ajax({
            ...
            $(ref.container).find('.best_practices_bg').find('.default-screen').find('.text-container-inside').scrollTop(0);
                ...
                var bestStr = '';
                if (boolValue == true) {
                    bestStr += '<div class="col-xs-6 col-sm-5 col-md-5 col-lg-5 img-tag"><img class="img-responsive desktop-bg-img" src="' + $(xml).find('best_practices_main').find('popupContent').find('item').find('image_desktop').text() + '"><img class="img-responsive mobile-bg-img hide" src="' + $(xml).find('best_practices_main').find('popupContent').find('item').find('image_mobile').text() + '"></div>';
                    bestStr += '<div class="col-xs-6 col-sm-6 col-md-7 col-lg-7 text-tag"><div class="textContent">' + $(xml).find('best_practices_main').find('popupContent').find('item').find('content').text() + '</div></div>';
                }
                ...
                var possibleStr = '';
                if (boolValue == true) {
                    possibleStr += '<div class="col-xs-6 col-sm-5 col-md-5 col-lg-5 img-tag"><img class="img-responsive desktop-bg-img" src="' + $(xml).find('best_practices_main').find('popupContent').find('itemP').find('image_desktop').text() + '"><img class="img-responsive mobile-bg-img hide" src="' + $(xml).find('best_practices_main').find('popupContent').find('itemP').find('image_mobile').text() + '"></div>';
                    possibleStr += '<div class="col-xs-6 col-sm-6 col-md-7 col-lg-7 text-tag"><div class="textContent">' + $(xml).find('best_practices_main').find('popupContent').find('itemP').find('content').text() + '</div></div>';
                }
